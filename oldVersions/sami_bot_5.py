...
class SAMIBot:
    ...
    def chat(self, user_input):
        ...
        try:
            ...
            end_time = time.time()
            print(f"[DEBUG] Respuesta recibida en {end_time - start_time:.2f} segundos")
...
def test_connection():
    ...
    try:
        import requests
        # vLLM usa el endpoint /v1/models para verificar conexión
        ...
        if response.status_code == 200:
            ...
            print(f"   Modelos disponibles: {len(models.get('data', []))} modelo(s)")
            return True
        else:
            print(f"❌ Servidor vLLM responde con código {response.status_code}")
            return False
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'debug':
                ...
                print(f"[INFO] El servidor se verificó al iniciar el programa\n")
                continue
